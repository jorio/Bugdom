...
{
	if (setupInfo == nil)
		DoFatalAlert("\pQD3D_CalcObjectBoundingSphere: setupInfo = nil");
	if (theObject == nil)
		DoFatalAlert("\pQD3D_CalcObjectBoundingSphere: theObject = nil");
	if (sphere == nil)
		DoFatalAlert("\pQD3D_CalcObjectBoundingSphere: sphere = nil");
...
int		i, numPoints;
	...
	if (outData->triangles == nil)
		DoFatalAlert("\pQD3D_CopyTriMeshData: out of memory copying triangles");
	...
	if (outData->points == nil)
		DoFatalAlert("\pQD3D_CopyTriMeshData: out of memory copying points");
	...
	if (inData->numVertexAttributeTypes)
	{
		...
		if (outData->vertexAttributeTypes == nil)
			DoFatalAlert("\pQD3D_CopyTriMeshData: out of memory copying vertex attrib types");
	...
	outData->vertexAttributeTypes[0].data = (void *)AllocPtr(sizeof(TQ3Vector3D) * numPoints);				// set new data array
	if (outData->vertexAttributeTypes[0].data == nil)
		DoFatalAlert("\pQD3D_CopyTriMeshData: no mem for vert normal attribs");
	...
	if (outData->numVertexAttributeTypes > 1)
	{
		...
		outData->vertexAttributeTypes[1].data = (void *)AllocPtr(sizeof(TQ3Param2D) * numPoints);			// set new data array
		if (outData->vertexAttributeTypes[1].data == nil)
			DoFatalAlert("\pQD3D_CopyTriMeshData: no mem for vert UV attribs");
